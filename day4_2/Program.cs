...
namespace day4_2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var guardMinute = guardBehavior.Select(gb => (gb.Key, GetLongestSleep(gb.Value)))
                                           .OrderByDescending(gb => gb.Item2.timeSpent)
                                           .First();
            
            var solution = guardMinute.Item1 * guardMinute.Item2.minute;
        ...
        static (int timeSpent, int minute) GetLongestSleep(int[] minutes){
            return minutes.Zip(Enumerable.Range(0, 59), (m, p) => (m, p))
